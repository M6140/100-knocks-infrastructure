...
項目名: [Knowledge] プロセスの優先度管理（nice/renice）
内容:

nice 値の範囲は -20（最高）〜 19（最低）。

数値を大きくすると「いい人（nice）」になり、他のプロセスに CPU を譲る。

実務で重いバッチ処理を流すときは、nice 値を大きくしてシステム全体の停止を防ぐ。


「システムが重いとき、優先度を上げる（数値を小さくする）のと、下げる（数値を大きくする）のでは、どちらが『行儀が良い（他を優先させる）』設定ですか？」
優先度を下げます。


[Verification] Webサーバーの正常動作確認

curl localhost にて HTML の応答を確認。

ドキュメントルート: /var/www/html/

アクセスログの場所: /var/log/httpd/access_log

学び: systemctl が機能しない環境でも、バイナリ直接起動と ss コマンドによるポート確認ができれば、サービスの構築とトラブルシューティングは可能である。


journalctl で『前回の再起動（1つ前のブート）より前のログ』を見たいとき、どのオプションを使いますか？
-r　→　-b -1